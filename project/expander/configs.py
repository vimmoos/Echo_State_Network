...
final_reservoir = {
    "spectral_radius": ((x / 5) + 0.5 for x in range(5)),
    "density": [0.04],
...
esn_gen = {
    "data": [
        c.Data(np.array(list(~(data.all * 50))), (data.all * 50).tempo, 1280,
               5500, 5500)
    ],
    "in_out": [9],
    "reg": [1e-8],
...
esn_final = {
    "data": [
        c.Data(np.array(list(~(data.all * 10))), (data.all * 10).tempo, 3200,
               6200, 6200)
    ],
    "matrix_path":
...
if __name__ == "__main__":
    ...
    p = e.esn_pickler(esn_final,
                      path_to_dir="/home/vimmoos/NN/resources/esn/final_final/",
                      verbose=True)
    p()
