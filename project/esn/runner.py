...
class Runner:
    ...
    inputs: np.ndarray = field(default_factory=lambda: np.zeros((0, 0)))
    """ the input of the runnner"""
    outputs: np.ndarray = field(default_factory=lambda: np.zeros((0, 0)))
    """the possible output"""
...
def runner(updator: up.Updator, inputs, outputs=None):
    ...
    try:
        while True:
            ...
            (u, o) = next(gen) if new_uo is None else new_uo
