...
def experiment(path):
    """given a path to a directory take all the file in it and returns a generator
...
def get_data (paths):
    """load each file given a list or a generator of path
...
def apply_metrics(output, desired, raw_output):
    """apply all the possible metrics usign the output and the desired
...
def apply_transformers(dict_, data_len):
    """apply each tranformer to the output and then apply each metrics to
each output generated by the transformers
...
def remove_raw(data_gen, rkeys=["output", "desired", "input"]):
    """remove the raw data from the data generator
...
def process_data(data, data_len, rkeys=None):
    """post process all the raw data given
...
def hget_metrics(metrics):
    """returns the headers for each metrics
...
def hget_params(params):
    """returns the headers for each parameter
...
def bget_tparam(metrics):
    """return all the paramater for a transformer
...
def bget_tsqueeze(metrics):
    """returns all the squeezing function for a transformer"""
    return [metrics[x]["squeeze_f"] for x in range(len(squeeze_fs))]


def bget_params(params):
    """returns all the parameter of the network
...
def get_body(single_run):
    """return a nested body which need to be flattened
...
def my_flatten(lol):
    """flatten function which return a lol given a nested list
...
def to_csv(single_run, file):
    """write a csv file for the single run
...
def peek(iterable):
    """utility function used to see if the generator has still something
...
def multiproc(func, args, workers, slicing=20):
    """spawn a pool of procees and apply the function to the arguments in
a current manner
...
def partition_data(graw_data, n):
    """partition a generator in batch (len(batch)==n)
...
def pre_post_proc(enumeration, cpath, data_len):
    """post process the data and dump a csv file sequentially
...
def cpre_post_proc(raw_data, size_group=4, workers=20):
    """perform the post process data with multiple process
